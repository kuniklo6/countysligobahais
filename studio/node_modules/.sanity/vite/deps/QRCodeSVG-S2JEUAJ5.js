...
// node_modules/sanity/lib/_chunks-es/QRCodeSVG.js
...
var QrCode = class _QrCode {
  ...
  static encodeSegments(segs, ecl, minVersion = 1, maxVersion = 40, mask = -1, boostEcl = true) {
    ...
    for (version = minVersion; ; version++) {
      const dataCapacityBits2 = _QrCode.getNumDataCodewords(version, ecl) * 8, usedBits = QrSegment.getTotalBits(segs, version);
    ...
    for (const newEcl of [Ecc.MEDIUM, Ecc.QUARTILE, Ecc.HIGH])
      boostEcl && dataUsedBits <= _QrCode.getNumDataCodewords(version, newEcl) * 8 && (ecl = newEcl);
    const bb = [];
    ...
    return bb.forEach((b, i) => dataCodewords[i >>> 3] |= b << 7 - (i & 7)), new _QrCode(version, ecl, dataCodewords, mask);
  }
  /*-- Fields --*/
  ...
  constructor(version, errorCorrectionLevel, dataCodewords, msk) {
    if (this.version = version, this.errorCorrectionLevel = errorCorrectionLevel, version < _QrCode.MIN_VERSION || version > _QrCode.MAX_VERSION) throw new RangeError("Version value out of range");
  ...
  getPenaltyScore() {
    ...
    for (let y = 0; y < this.size; y++) {
      ...
      for (let x = 0; x < this.size; x++)
        this.modules[y][x] == runColor ? (runX++, runX == 5 ? result += _QrCode.PENALTY_N1 : runX > 5 && result++) : (this.finderPenaltyAddHistory(runX, runHistory), runColor || (result += this.finderPenaltyCountPatterns(runHistory) * _QrCode.PENALTY_N3), runColor = this.modules[y][x], runX = 1);
    ...
    for (let x = 0; x < this.size; x++) {
      ...
      const runHistory = [0, 0, 0, 0, 0, 0, 0];
      for (let y = 0; y < this.size; y++)
        this.modules[y][x] == runColor ? (runY++, runY == 5 ? result += _QrCode.PENALTY_N1 : runY > 5 && result++) : (this.finderPenaltyAddHistory(runY, runHistory), runColor || (result += this.finderPenaltyCountPatterns(runHistory) * _QrCode.PENALTY_N3), runColor = this.modules[y][x], runY = 1);
      result += this.finderPenaltyTerminateAndCount(runColor, runY, runHistory) * _QrCode.PENALTY_N3;
    }
    for (let y = 0; y < this.size - 1; y++)
      for (let x = 0; x < this.size - 1; x++) {
        const color = this.modules[y][x];
        color == this.modules[y][x + 1] && color == this.modules[y + 1][x] && color == this.modules[y + 1][x + 1] && (result += _QrCode.PENALTY_N2);
      }
    let dark = 0;
    for (const row of this.modules) dark = row.reduce((sum, color) => sum + (color ? 1 : 0), dark);
    const total = this.size * this.size, k = Math.ceil(Math.abs(dark * 20 - total * 10) / total) - 1;
    return assert(0 <= k && k <= 9), result += k * _QrCode.PENALTY_N4, assert(0 <= result && result <= 2568888), result;
  }
  ...
  static reedSolomonComputeDivisor(degree) {
    ...
    for (let i = 0; i < degree; i++) {
      for (let j = 0; j < result.length; j++)
        result[j] = _QrCode.reedSolomonMultiply(result[j], root), j + 1 < result.length && (result[j] ^= result[j + 1]);
  ...
  static reedSolomonComputeRemainder(data, divisor) {
    ...
    for (const b of data) {
      ...
      result.push(0), divisor.forEach((coef, i) => result[i] ^= _QrCode.reedSolomonMultiply(coef, factor));
    }
    return result;
  }
...
var QrSegment = class _QrSegment {
  ...
  static makeAlphanumeric(text) {
    ...
    for (i = 0; i + 2 <= text.length; i += 2) {
      ...
      temp += _QrSegment.ALPHANUMERIC_CHARSET.indexOf(text.charAt(i + 1)), appendBits(temp, 11, bb);
    }
    return i < text.length && appendBits(_QrSegment.ALPHANUMERIC_CHARSET.indexOf(text.charAt(i)), 6, bb), new _QrSegment(Mode.ALPHANUMERIC, text.length, bb);
  }
  // Returns a new mutable list of zero or more segments to represent the given Unicode text string.
  // The result may use various segment modes and switch modes to optimize the length of the bit stream.
  static makeSegments(text) {
    return text == "" ? [] : _QrSegment.isNumeric(text) ? [_QrSegment.makeNumeric(text)] : _QrSegment.isAlphanumeric(text) ? [_QrSegment.makeAlphanumeric(text)] : [_QrSegment.makeBytes(_QrSegment.toUtf8ByteArray(text))];
  }
...
var Ecc = class _Ecc {
  /*-- Constants --*/
  static LOW = new _Ecc(0, 1);
  // The QR Code can tolerate about  7% erroneous codewords
  static MEDIUM = new _Ecc(1, 0);
  // The QR Code can tolerate about 15% erroneous codewords
  static QUARTILE = new _Ecc(2, 3);
  // The QR Code can tolerate about 25% erroneous codewords
  static HIGH = new _Ecc(3, 2);
...
var Mode = class _Mode {
  /*-- Constants --*/
  static NUMERIC = new _Mode(1, [10, 12, 14]);
  static ALPHANUMERIC = new _Mode(2, [9, 11, 13]);
  static BYTE = new _Mode(4, [8, 16, 16]);
  static KANJI = new _Mode(8, [8, 10, 12]);
  static ECI = new _Mode(7, [0, 0, 0]);
...
function excavateModules(modules, excavation) {
  return modules.slice().map((row, y) => y < excavation.y || y >= excavation.y + excavation.h ? row : row.map((cell, x) => x < excavation.x || x >= excavation.x + excavation.w ? cell : false));
}
...
function QRCodeSVGComponent(props) {
  ...
  let t8;
  $[13] !== title ? (t8 = !!title && (0, import_jsx_runtime.jsx)("title", { children: title }), $[13] = title, $[14] = t8) : t8 = $[14];
  let t10, t11, t9;
  ...
  let t12;
  $[18] !== color || $[19] !== fgPath ? (t12 = (0, import_jsx_runtime.jsx)(motion.path, { fill: color, d: fgPath, shapeRendering: "crispEdges", initial: t9, animate: t10, exit: t11 }), $[18] = color, $[19] = fgPath, $[20] = t12) : t12 = $[20];
  let t13;
  return $[21] !== size || $[22] !== t12 || $[23] !== t7 || $[24] !== t8 ? (t13 = (0, import_jsx_runtime.jsxs)("svg", { height: size, width: size, viewBox: t7, role: "img", children: [
    t8,
    t12
  ] }), $[21] = size, $[22] = t12, $[23] = t7, $[24] = t8, $[25] = t13) : t13 = $[25], t13;
}
var QRCodeSVG = (0, import_react2.memo)(QRCodeSVGComponent);
QRCodeSVG.displayName = "Memo(QRCodeSVG)";
export {
  QRCodeSVG as default
};
//# sourceMappingURL=QRCodeSVG-S2JEUAJ5.js.map
